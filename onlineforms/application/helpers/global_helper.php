	...
	if(!function_exists('__load_assets__')) {
		function __load_assets__($assets,$type){
			...
			foreach ($assets[$ci->router->fetch_class()][$type] as $key => $value) {
				if($type == 'css'){
					echo "<link rel='stylesheet' href='". base_url('assets/modules/' . $ci->router->fetch_class() . '/'.$type.'/') . $value . "?ver=".time()."' />";
				} else {
					echo "<script type='text/javascript' src='" . base_url('assets/modules/' . $ci->router->fetch_class() . '/' . $type . '/') . $value . "?ver=".time()."' ></script>";
				}
				echo "\r\n";
			}
	    }
