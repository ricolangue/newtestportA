...
function send_email($parameter = array(), $success_message = '', $error_message = '')
{
    if (function_exists('curl_version')) {
        ...
        if ($parameter['mail_type'] == 2) {
            $link = get_home_url() . '/onlineforms/' . $_SESSION['token'];
            // $parameter['link_url'] = get_home_url() . '/onlineforms/' . $_SESSION['token'];
            $parameter['link_url'] = '<a href="' . $link . '" target="_blank">' . $link . '</a>';
        }
        ...
        $template = (empty($parameter['debug'])) ? TEMPLATE_TEST : $parameter['debug'];
        $mode = ($template == true) ? 'test_send_email' : 'send_email';
        ...
        if ($response['response'] == 'sent') {
            if (!empty($success_message)) {
                ...
                return '<div id="success"><div class="message animated-success"><span>THANK YOU</span><br/> <span>for sending us a message!</span><br/><span>We will be in touch with you soon.</span><p class="close">x</p></div></div>';
                ;
            }
            ...
            if (!empty($error_message)) {
                ...
                return '<div id="error-msg"><div class="message animated-error"><span>Failed to send email. Please try again.</span><br/><p class="error-close">x</p></div></div>';
            }
            unset($_POST);
        }
        curl_close($ch);
